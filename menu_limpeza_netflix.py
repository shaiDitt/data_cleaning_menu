...
"""
    TODO - REMOVER O COMENTARIO DEPOIS DE FINALIZADO
    df = load_ficheiro_excel()
    Este df acima faz o load de forma dinamica
"""
...
def load_ficheiro_excel():
    global caminho_ficheiro
    while True:
        caminho_ficheiro = ler_string(
            "Qual o caminho do ficheiro que deseja carregar? (ou digite 'sair' para cancelar): "
        )

        if caminho_ficheiro == None:
            print("Operação cancelada.")
            break

        # 1. Permite ao utilizador sair a qualquer momento
        if caminho_ficheiro.strip().lower() == 'sair':
            print("Operação cancelada pelo utilizador.")
            return None
        
        caminho = Path(caminho_ficheiro)
        # 2. se não é um ficheiro, tenta novamente
        if not caminho.is_file():
            print(f"Erro: Ficheiro não encontrado em '{caminho_ficheiro}'. Tente novamente.")
            continue
        ...
        if extensao == '.csv':
            try:
                print("A ler o ficheiro")
                return pd.read_csv(caminho_ficheiro)
            except:
                print("Erro ao ler ficheiro CSV")
                continue

        # 4. Processamento do Excel
        elif extensao in ['.xlsx', '.xls']:
            try:
                # 1. Abre a estrutura do Excel para inspecionar as folhas rapidamente
                excel_doc = pd.ExcelFile(caminho_ficheiro)
                folhas_disponiveis = excel_doc.sheet_names

                print(f"\nFolhas encontradas no ficheiro: {folhas_disponiveis}")
                
                # 2. Pergunta ao utilizador qual a folha que deseja
                escolha_folha = ler_string("Escreva o nome da folha pretendida, prima Enter para carregar TODAS (ou digite 'sair' para cancelar): ").strip()

                if escolha_folha.lower() == 'sair':
                    print("Operação cancelada pelo utilizador.")
                    return None

                # 3. Lógica de escolha
                if escolha_folha == "":
                    print("Todas as folhas foram carregadas com sucesso!")
                    return pd.read_excel(excel_doc, sheet_name=None)
                
                elif escolha_folha in folhas_disponiveis:
                    # Carrega apenas a folha escolhida -> devolve um DataFrame
                    print(f"A folha {escolha_folha} foi carregada com sucesso!")
                    return pd.read_excel(excel_doc, sheet_name=escolha_folha)
                
                else:
                    print(f"Erro: A folha '{escolha_folha}' não existe neste ficheiro. Tente novamente.\n")
                    continue
        ...
        # 5. Formato inválido
            ...
            print(f"Erro: Formato '{extensao}' não suportado. Use apenas .csv, .xlsx ou .xls\n")
            continue
...
def excluir_coluna(df):
   ...
   if confirmar == "s":
       ...
       salvar_operacao_realizada(f"A coluna '{coluna_escolhida}' foi excluída.")
...
def filtro(df):
    ...
    print(colunas)
    print(56*"-")
    coluna = ler_string("Qual coluna deseja utilizar como filtro? ").strip()
...
def valores_unicos(df):
    ...
    print("╚══════════════════════════════════════════════════════════════╝")
    coluna = ler_string("Verificar qual coluna? ").strip()
    if coluna not in df.columns:
        print("Erro: A coluna não existe no DataFrame ou não tem números em falta.")
        return df
    else:
...
def adicionar_linha(df, caminho_excel):
    ...
    for coluna in df.columns:
        if coluna == "show_id":
            print("Coluna 'show_id é a chave primária, adicionada automaticamente")
            prima_enter()
            clean()
...
def editar_celula(df):
    ...
    print("╚══════════════════════════════════════════════════════════════╝")
    mostrar_colunas_existentes(df)
    coluna = ler_string("Insira qual a coluna que você quer editar: ").strip()
